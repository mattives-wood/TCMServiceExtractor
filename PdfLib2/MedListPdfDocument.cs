...
namespace PdfLib;
public class MedListPdfDocument
{
    private readonly string _path;
    private readonly MetadataContext _metadataContext;

    public MedListPdfDocument(string path, MetadataContext metadataContext)
    {
        _metadataContext = metadataContext;
        _path = path;
    }

    public void Render(Client client, List<Medication> meds, MedInfo medInfo)
    {
        ...
        if (medInfo != null)
        {
            ProcessMedInfo(doc, medInfo);
        }
        ProcessMedList(doc, meds);
    ...
    private void ProcessMedInfo(Document doc, MedInfo medInfo)
    {
    ...
    private void ProcessMedList(Document doc, List<Medication> meds)
    {
        foreach (Medication med in meds.OrderBy(m => m.StartDate))
        {
            ...
            Paragraph paragraph = doc.LastSection
                                    .AddParagraph(bookmark.ToString(), "Heading1");
